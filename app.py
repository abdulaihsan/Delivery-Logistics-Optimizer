...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        st.markdown("<h1 style='text-align: center; color: #FDBA74;'>🔒 Evo Routes Access</h1>", unsafe_allow_html=True)
        st.text_input(
            "Enter Security Code", 
            type="password", 
...
if "Linear" in model_choice:
    st.sidebar.markdown("🔹 **Performance:** High Speed, High Interpretability\n\n📉 **RMSE:** 8.37 min\n\n📈 **R²:** 0.84")
elif "Random Forest" in model_choice:
    st.sidebar.markdown("🔹 **Performance:** Non-linear capability, Higher Latency\n\n📉 **RMSE:** 11.46 min\n\n📈 **R²:** 0.70")
else:
    st.sidebar.markdown("🔹 **Performance:** Experimental Traffic Modeling\n\n📉 **RMSE:** 13.1 min\n\n📈 **R²:** 0.61")
...
if not uploaded_file:
    ...
    if 'LATITUDE' not in df.columns or 'LONGITUDE' not in df.columns:
        st.warning("⚠️ CSV lacks 'Latitude'/'Longitude'. Generating simulated coordinates.")
...
with col1:
    ...
    optimize_btn = st.button("🚀 EXECUTE OPTIMIZATION", type="primary", use_container_width=True)
...
def run_optimization_simulation(df, model_type):
    ...
    with st.spinner(f'🔄 Evolving Optimal Paths...'):
